...
def _get_market_caps(symbols: Iterable[str], cache_dir: Path) -> Dict[str, float]:
    ...
    for symbol in symbols:
        if symbol in cache:
            ...
            continue
        try:
            # Use Financial Modeling Prep for market cap
            api_key = os.getenv("FMP_API_KEY")
            if not api_key:
                # Fallback to yfinance if no FMP key
                ...
                if response.status_code == 200:
                    data = response.json()
                    cap = data[0].get("mktCap", 0) if isinstance(data, list) else data.get("mktCap", 0)
                else:
                    cap = 0.0
...
def _get_earnings_dates(symbol: str, start: pd.Timestamp, end: pd.Timestamp, cache_dir: Path) -> List[pd.Timestamp]:
    ...
    try:
        ...
        print(f"Error fetching earnings from FMP for {symbol}: {e}")
...
def run_earnings_backtest(
    ...
    # Keep earnings/market-cap caches inside the backtesting folder when possible.
    # Otherwise, these relative paths can pollute the top-level repo ./data directory.
    bt_root = output_dir
    while bt_root.name != "backtesting" and bt_root.parent != bt_root:
        bt_root = bt_root.parent
    if bt_root.name == "backtesting":
        cache_dir = bt_root / "data" / "cache"
    else:
        # Fallbacks
        cache_dir = Path("backtesting") / "data" / "cache"
