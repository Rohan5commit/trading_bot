...
def _build_daily_command(config, *, service_port: int, reset_ai_positions: bool, clear_email_markers: bool) -> str:
    ...
    post_script = "\n".join(
        [
            ...
            "already_emailed_skip = ('email already sent' in text_lower and 'skipping run' in text_lower)",
            ...
            "ai_path_ok = bool(model_call_observed or ai_skipped_legit or already_emailed_skip)",
            "run_rc = int(os.getenv('DAILY_RUN_RC', '1') or '1')",
            "result = {",
            "    'ok': bool(",
            "        run_rc == 0",
            "        and (email_sent_count >= required_email_count or already_emailed_skip)",
            ...
            "    'already_emailed_skip': already_emailed_skip,",
